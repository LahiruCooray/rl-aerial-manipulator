...
def main():
    ...
    try:
        model = PPO.load("waypoint_controller_vecNormalized", custom_objects={
    "clip_range": 0.2,        
    "lr_schedule": 0.0003     
})
    ...
    waypoints_holder = [np.array(env.envs[0].waypoint_list)]
    ...
    def control_loop(i):
        ...
        for _ in range(4):  # Same as original runsim.py
            if step_count >= max_steps:
                ...
                waypoints_holder[0] = np.array(env.envs[0].waypoint_list)
                print(f"Waypoints to visit: {waypoints_holder[0]}")
                step_count = 0
                break
