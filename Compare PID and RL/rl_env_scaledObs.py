...
class WaypointQuadEnv(gym.Env):
    def __init__(self, start_pos=None, target_pos=None):
        super().__init__()
        # Allow user to specify initial and target point
        self.start_pos = np.array(start_pos) if start_pos is not None else np.array([0.5, -0.5, 2.0])
        self.target_pos = np.array(target_pos) if target_pos is not None else np.array([-0.5, 0.5, 5.0])
    ...
    def reset(self, seed=None, start_pos=None, target_pos=None):
        super().reset(seed=seed)
        # Use provided or default start/target
        ...
        self.quadcopter = Quadcopter(self.start_pos, start_orientation)

        self.current_step = 0
        self.max_episode_steps = 5000
        self.counter = 0
        self.counter_activated = False
        self.counter_limit = 5000

        # Set waypoint list to only the target
        self.waypoint_list = [self.target_pos]
